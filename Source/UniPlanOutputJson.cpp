...
int RunListJson(const Inventory &InInventory, const std::string &InKind,
  ...
  if (InKind == "pair") {
    ...
    PrintJsonHeader(kPairListSchema, InInventory.mGeneratedUtc,
                    InInventory.mRepoRoot);
    if (bFilterActive) {
    ...
    for (size_t Index = 0; Index < FilteredPairs.size(); ++Index) {
      ...
      std::cout << "\"implementation_status\":"
                << JsonQuote(GetDisplayStatus(Pair.mImplementationStatus))
                << ",";
      std::cout << "\"playbooks\":[";
      for (size_t PlaybookIndex = 0; PlaybookIndex < Pair.mPlaybooks.size();
           ++PlaybookIndex) {
      ...
      EmitJsonField("overall_status", GetDisplayStatus(Pair.mOverallStatus),
                    false);
      std::cout << "}";
    }
    std::cout << "],";
  ...
  PrintJsonHeader(kListSchema, InInventory.mGeneratedUtc,
                  InInventory.mRepoRoot);
...
int RunLintJson(const LintResult &InResult) {
  ...
  std::cout << ",\"checks\":[";
  std::cout << "{\"id\":\"name_pattern\",\"warning_count\":"
            << InResult.mNamePatternWarningCount << "},";
  std::cout << "{\"id\":\"missing_h1\",\"warning_count\":"
            << InResult.mMissingH1WarningCount << "}";
  std::cout << "]}\n";
  return 0;
}
...
int RunOrphanCheckJson(const OrphanCheckResult &InResult) {
  PrintJsonHeader(kOrphanCheckSchema, InResult.mGeneratedUtc,
                  InResult.mRepoRoot);
...
int RunArtifactsJson(const Inventory &InInventory,
  ...
  const bool IncludeImplementation =
      (InKind == "all" || InKind == "implementation");
  ...
  const auto FindSidecarPath =
      [&Sidecars](const std::string &InOwnerKind, const std::string &InDocKind,
                  const std::string &InPhaseKey) -> std::string {
    for (const SidecarRecord &Sidecar : Sidecars) {
      if (Sidecar.mOwnerKind == InOwnerKind && Sidecar.mDocKind == InDocKind &&
          (InPhaseKey.empty() || Sidecar.mPhaseKey == InPhaseKey)) {
  ...
  PrintJsonHeader(kArtifactsSchema, InInventory.mGeneratedUtc,
                  InInventory.mRepoRoot);
  EmitJsonField("topic_key", InTopicKey);
  std::cout << "\"bundle\":{";
  EmitJsonField("pair_state",
  ...
  if (!IncludeSidecar) {
    ...
    std::cout << "{";
    std::cout << "\"plan\":{";
    EmitJsonFieldNullable("changelog",
                          FindSidecarPath("Plan", kSidecarChangeLog, ""));
    EmitJsonFieldNullable("verification",
                          FindSidecarPath("Plan", kSidecarVerification, ""),
                          false);
    std::cout << "},";
    std::cout << "\"implementation\":{";
    EmitJsonFieldNullable("changelog",
                          FindSidecarPath("Impl", kSidecarChangeLog, ""));
    EmitJsonFieldNullable("verification",
                          FindSidecarPath("Impl", kSidecarVerification, ""),
                          false);
    std::cout << "},";
    std::cout << "\"playbooks\":[";
    bool FirstSidecarPlaybook = true;
    for (const DocumentRecord &Playbook : Playbooks) {
      const std::string PlaybookChangeLog =
          FindSidecarPath("Playbook", kSidecarChangeLog, Playbook.mPhaseKey);
      const std::string PlaybookVerification =
          FindSidecarPath("Playbook", kSidecarVerification, Playbook.mPhaseKey);
...
int RunEvidenceJson(const char *InSchemaId, const std::string &InGeneratedUtc,
  ...
  for (size_t EntryIndex = 0; EntryIndex < InEntries.size(); ++EntryIndex) {
    ...
    for (size_t FieldIndex = 0; FieldIndex < Entry.mFields.size();
      ...
      std::cout << JsonQuote(Entry.mFields[FieldIndex].first) << ":"
                << JsonQuote(Entry.mFields[FieldIndex].second);
    }
    std::cout << "}}";
  }
  std::cout << "],";
...
int RunSchemaJson(const std::string &InGeneratedUtc,
                  ...
                  const std::vector<std::string> &InWarnings) {
  PrintJsonHeader(kSchemaSchema, InGeneratedUtc, InRepoRoot);
...
int RunRulesJson(const std::string &InGeneratedUtc,
  ...
  std::cout << "\"provenance_unresolved_count\":"
...
int RunSectionSchemaJson(const std::string &InGeneratedUtc,
  ...
  for (size_t Index = 0; Index < InEntries.size(); ++Index) {
    ...
    std::cout << "{\"section_id\":" << JsonQuote(Entry.mSectionId)
              << ",\"required\":" << (Entry.mbRequired ? "true" : "false")
              << ",\"order\":" << Entry.mOrder << "}";
  }
  std::cout << "],";
...
int RunSectionListJson(const std::string &InGeneratedUtc,
  ...
  for (size_t Index = 0; Index < InCounts.size(); ++Index) {
    ...
    std::cout << "{\"heading\":" << JsonQuote(Item.mHeading)
              << ",\"section_id\":" << JsonQuote(Item.mSectionId);
    if (InShowCount) {
...
int RunSectionListDocJson(const std::string &InGeneratedUtc,
  ...
  for (size_t Index = 0; Index < InHeadings.size(); ++Index) {
    ...
    std::cout << "{\"section_id\":" << JsonQuote(Heading.mSectionId)
              << ",\"heading\":" << JsonQuote(Heading.mText)
              << ",\"level\":" << Heading.mLevel
              << ",\"line\":" << Heading.mLine << "}";
  }
  std::cout << "],";
  std::vector<std::string> Warnings;
...
int RunTableGetJson(const std::string &InGeneratedUtc,
  ...
  for (size_t RowIndex = 0; RowIndex < InTable.mRows.size(); ++RowIndex) {
    ...
    std::cout << "[";
    for (size_t CellIndex = 0; CellIndex < InTable.mRows[RowIndex].size();
         ++CellIndex) {
  ...
  for (size_t RowIndex = 0; RowIndex < InTable.mRows.size(); ++RowIndex) {
    ...
    for (size_t HeaderIndex = 0; HeaderIndex < InTable.mHeaders.size();
      ...
      const std::string Value = (HeaderIndex < InTable.mRows[RowIndex].size())
                                    ? InTable.mRows[RowIndex][HeaderIndex]
                                    : "";
      std::cout << JsonQuote(InTable.mHeaders[HeaderIndex]) << ":"
                << JsonQuote(Value);
    }
    std::cout << "}";
  }
...
int RunPhaseJson(const std::string &InGeneratedUtc,
  ...
  for (size_t Index = 0; Index < InItems.size(); ++Index) {
    ...
    std::cout << "\"fields\":{";
    for (size_t FieldIndex = 0; FieldIndex < Item.mFields.size();
         ++FieldIndex) {
      PrintJsonSep(FieldIndex);
      std::cout << JsonQuote(Item.mFields[FieldIndex].first) << ":"
                << JsonQuote(Item.mFields[FieldIndex].second);
    }
    std::cout << "}}";
  }
  std::cout << "],";
...
int RunSectionContentJson(const std::string &InGeneratedUtc,
  ...
  for (size_t TableIndex = 0; TableIndex < InTables.size(); ++TableIndex) {
    ...
    for (size_t RowIndex = 0; RowIndex < Table.mRows.size(); ++RowIndex) {
      PrintJsonSep(RowIndex);
      std::cout << "[";
      for (size_t CellIndex = 0; CellIndex < Table.mRows[RowIndex].size();
           ++CellIndex) {
        PrintJsonSep(CellIndex);
