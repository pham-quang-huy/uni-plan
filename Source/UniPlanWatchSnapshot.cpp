...
static FWatchPhaseTaxonomy BuildPhaseTaxonomy(const PhaseItem &InPhase,
                                              const fs::path &InRepoRoot) {
  FWatchPhaseTaxonomy Taxonomy;
    ...
    for (const std::vector<std::string> &Row : Table.mRows) {
      ...
      Lane.mStatus =
          (StatusCol >= 0 && StatusCol < static_cast<int>(Row.size()))
              ...
              : "unknown";
      Lane.mScope = (ScopeCol >= 0 && ScopeCol < static_cast<int>(Row.size()))
  ...
  for (const MarkdownTableRecord &Table : Tables) {
    ...
    int WaveCol = -1, LaneCol = -1, JobCol = -1, StatusCol = -1, ScopeCol = -1,
        ExitCol = -1;
    for (int Col = 0; Col < static_cast<int>(Table.mHeaders.size()); ++Col) {
      const std::string Lower =
          ToLower(Trim(Table.mHeaders[static_cast<size_t>(Col)]));
    ...
    for (const std::vector<std::string> &Row : Table.mRows) {
      FWatchJobItem Job;
      Job.mWaveID = (WaveCol >= 0 && WaveCol < static_cast<int>(Row.size()))
                        ? Trim(Row[static_cast<size_t>(WaveCol)])
                        : "";
      Job.mLaneID = (LaneCol >= 0 && LaneCol < static_cast<int>(Row.size()))
                        ? Trim(Row[static_cast<size_t>(LaneCol)])
                        : "";
      std::string JobCell =
      ...
      Job.mStatus =
          (StatusCol >= 0 && StatusCol < static_cast<int>(Row.size()))
              ? NormalizeStatusValue(Row[static_cast<size_t>(StatusCol)])
              : "unknown";
      Job.mScope = (ScopeCol >= 0 && ScopeCol < static_cast<int>(Row.size()))
                       ? Trim(Row[static_cast<size_t>(ScopeCol)])
                       : "";
      Job.mExitCriteria =
          (ExitCol >= 0 && ExitCol < static_cast<int>(Row.size()))
  ...
  for (const MarkdownTableRecord &Table : Tables) {
    ...
    for (const std::vector<std::string> &Row : Table.mRows) {
      FWatchTaskItem Task;
      Task.mJobRef = (JobCol >= 0 && JobCol < static_cast<int>(Row.size()))
                         ? Trim(Row[static_cast<size_t>(JobCol)])
                         : "";
      Task.mTaskID = (TaskCol >= 0 && TaskCol < static_cast<int>(Row.size()))
  ...
  for (const MarkdownTableRecord &Table : Tables) {
    ...
    for (const std::vector<std::string> &Row : Table.mRows) {
      const std::string FilePath = (FileCol < static_cast<int>(Row.size()))
                                       ? Trim(Row[static_cast<size_t>(FileCol)])
                                       : "";
      if (FilePath.empty() || FilePath == "N/A") {
        continue;
      }
      FWatchFileManifestItem Item;
...
BuildDocSchemaResult(const fs::path &InRepoRoot, const std::string &InDocPath,
  ...
  for (const DocHeading &DH : DocHeadings) {
    ...
    if (DH.mLevel == 2 && CanonicalIds.count(DH.mSectionId) > 0) {
      ...
      // H3 child heading — always canonical (legitimate child of parent H2)
      FWatchHeadingCheck Check;
      Check.mSectionId = DH.mSectionId;
      Check.mbPresent = true;
      Check.mbCanonical = true;
...
BuildTopicSchemaResult(const fs::path &InRepoRoot,
  ...
  // Plan schema
  Result.mPlan = BuildDocSchemaResult(InRepoRoot, InSummary.mPlanPath, "plan");
  ...
  for (const SidecarRecord &Sidecar : InInventory.mSidecars) {
    ...
    if (Owner == "Plan" && Kind == "ChangeLog") {
      Result.mPlanChangeLog =
          BuildDocSchemaResult(InRepoRoot, Sidecar.mPath, "plan_changelog");
    } else if (Owner == "Plan" && Kind == "Verification") {
      Result.mPlanVerification =
          BuildDocSchemaResult(InRepoRoot, Sidecar.mPath, "plan_verification");
    } else if (Owner == "Impl" && Kind == "ChangeLog") {
      Result.mImplChangeLog =
          BuildDocSchemaResult(InRepoRoot, Sidecar.mPath, "impl_changelog");
    } else if (Owner == "Impl" && Kind == "Verification") {
      Result.mImplVerification =
          BuildDocSchemaResult(InRepoRoot, Sidecar.mPath, "impl_verification");
    } else if (Owner == "Playbook" && Kind == "ChangeLog") {
      Result.mPlaybookChangeLogs.push_back(BuildDocSchemaResult(
          InRepoRoot, Sidecar.mPath, "playbook_changelog", Sidecar.mPhaseKey));
    } else if (Owner == "Playbook" && Kind == "Verification") {
      Result.mPlaybookVerifications.push_back(
          BuildDocSchemaResult(InRepoRoot, Sidecar.mPath,
                               "playbook_verification", Sidecar.mPhaseKey));
    }
  }
...
BuildPlanSummary(const PhaseListAllEntry &InEntry, const fs::path &InRepoRoot,
  ...
  // Extract plan summary, goals, and non-goals from plan document
  {
    ...
    if (TryReadFileLines(PlanAbsPath, PlanLines, PlanReadError)) {
      ...
      // Extract summary section text (lines between ## summary and next H2)
      ...
      // Strategy 1: goals_and_non_goals section table (Type/Statement cols)
    ...
    if (TryReadFileLines(SidecarAbsPath, SidecarLines, SidecarReadError)) {
      ...
      for (const MarkdownTableRecord &Table : SidecarTables) {
        if (Table.mSectionId == "entries" ||
          ...
          SidecarSummary.mEntryCount = static_cast<int>(Table.mRows.size());
          if (!Table.mRows.empty() && !Table.mRows.back().empty()) {
            SidecarSummary.mLatestDate = Trim(Table.mRows.back()[0]);
          }
          break;
        }
      }
  ...
  for (const PhaseItem &Phase : InEntry.mPhases) {
    if (!Phase.mPlaybookPath.empty()) {
      Summary.mPhaseTaxonomies.push_back(BuildPhaseTaxonomy(Phase, InRepoRoot));
    }
  }
...
FDocWatchSnapshot BuildWatchSnapshot(const std::string &InRepoRoot,
  ...
  Snapshot.mInventory.mPlaybookCount = static_cast<int>(Inv.mPlaybooks.size());
  ...
  for (const FWatchPlanSummary &Plan : Snapshot.mActivePlans) {
    Snapshot.mAllBlockers.insert(Snapshot.mAllBlockers.end(),
                                 Plan.mBlockers.begin(), Plan.mBlockers.end());
  }

  // 7. Measure poll duration
  const auto EndTime = std::chrono::steady_clock::now();
  Snapshot.mPollDurationMs = static_cast<int>(
