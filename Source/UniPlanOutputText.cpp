...
int RunListText(const Inventory& InInventory, const std::string& InKind, const std::string& InStatusFilter)
{
    const bool bFilterActive = (InStatusFilter != "all");

    if (InKind == "pair")
    {
        std::cout << "TopicKey | PairState | OverallStatus | Plan | PlanStatus | Implementation | ImplementationStatus | Playbooks\n";
        int Count = 0;
        for (const TopicPairRecord& Pair : InInventory.mPairs)
        {
            ...
            std::cout << Pair.mTopicKey << " | " << Pair.mPairState << " | " << (GetDisplayStatus(Pair.mOverallStatus)) << " | " << (Pair.mPlanPath.empty() ? "-" : Pair.mPlanPath) << " | " << (GetDisplayStatus(Pair.mPlanStatus)) << " | "
                      << (Pair.mImplementationPath.empty() ? "-" : Pair.mImplementationPath) << " | " << (GetDisplayStatus(Pair.mImplementationStatus)) << " | " << (PlaybookSummary.str().empty() ? "-" : PlaybookSummary.str()) << "\n";
        }
    ...
    for (const DocumentRecord* RecordPtr : FilteredRecords)
    {
        ...
        std::cout << " (status=" << (GetDisplayStatus(Record.mStatus)) << ", pair_state=" << PairState << "): " << Record.mPath << "\n";
    }
    return 0;
}
...
int RunInventoryText(const InventoryResult& InResult)
{
    ...
    {
        std::cout << std::left << std::setw(90) << Item.mPath << " " << std::setw(8) << Item.mLineCount << " " << Item.mLastCommit << "\n";
    }
...
int RunArtifactsText(const Inventory& InInventory, const std::string& InTopicKey, const std::string& InKind)
{
    ...
    std::cout << "Kind: " << InKind << "\n";
    std::cout << "PairState: " << ResolvePairStateForTopic(InInventory, InTopicKey) << "\n";
    if ((InKind == "all" || InKind == "plan") && Plan != nullptr)
    {
        std::cout << "Plan: " << Plan->mPath << " (status=" << (GetDisplayStatus(Plan->mStatus)) << ")\n";
    }
    if ((InKind == "all" || InKind == "implementation") && Implementation != nullptr)
    {
        std::cout << "Implementation: " << Implementation->mPath << " (status=" << (GetDisplayStatus(Implementation->mStatus)) << ")\n";
    }
    if (InKind == "all" || InKind == "playbook")
    {
        ...
        {
            std::cout << "- [" << Playbook.mPhaseKey << "] " << Playbook.mPath << " (status=" << (GetDisplayStatus(Playbook.mStatus)) << ")\n";
        }
    }
    if (InKind == "all" || InKind == "sidecar")
    {
        ...
        {
            std::cout << "- " << Sidecar.mOwnerKind << " " << (Sidecar.mPhaseKey.empty() ? "-" : Sidecar.mPhaseKey) << " " << Sidecar.mDocKind << ": " << Sidecar.mPath << "\n";
        }
    }
...
int RunEvidenceText(const std::string& InLabel, const std::string& InTopicKey, const std::string& InDocClass, const std::vector<EvidenceEntry>& InEntries, const std::vector<std::string>& InWarnings)
{
    std::cout << InLabel << " topic=" << InTopicKey << " doc_class=" << InDocClass << " entries=" << InEntries.size() << "\n";
...
int RunSchemeText(const std::string& InType, const std::vector<SchemaField>& InFields, const std::vector<std::string>& InExamples, const std::vector<std::string>& InWarnings)
{
    std::cout << "Schema type: " << InType << "\n";
    std::cout << "Fields: " << InFields.size() << "\n";
    for (const SchemaField& Field : InFields)
    {
        std::cout << "- [" << (Field.mSectionId.empty() ? "-" : Field.mSectionId) << "] " << Field.mProperty << " = " << Field.mValue << "\n";
    }
    std::cout << "Examples:\n";
...
int RunRulesText(const std::vector<RuleEntry>& InRules, const std::vector<std::string>& InWarnings)
{
    int ResolvedCount = 0;
    ...
    std::cout << "Rules: " << InRules.size() << " (provenance_resolved=" << ResolvedCount << ", unresolved=" << (static_cast<int>(InRules.size()) - ResolvedCount) << ")\n";
    for (const RuleEntry& Rule : InRules)
    {
        std::cout << "- [" << Rule.mId << "] " << Rule.mDescription << " (source=" << Rule.mSource;
        if (Rule.mbSourceResolved)
        {
            std::cout << ", table_id=" << Rule.mSourceTableId << ", row=" << Rule.mSourceRowIndex;
        }
        std::cout << ")\n";
    }
    PrintTextWarnings(InWarnings);
...
int RunValidateText(const bool InStrict, const bool InOk, const std::vector<ValidateCheck>& InChecks, const std::vector<std::string>& InErrors, const std::vector<std::string>& InWarnings)
{
    std::cout << "Validate: " << (InOk ? "ok" : "failed") << " (strict=" << (InStrict ? "true" : "false") << ")\n";
    for (const ValidateCheck& Check : InChecks)
    {
        std::cout << "- " << Check.mId << " " << (Check.mbOk ? "ok" : "failed") << " (critical=" << (Check.mbCritical ? "true" : "false");
...
int RunSectionResolveText(const std::string& InDocPath, const SectionResolution& InResolution, const std::vector<std::string>& InWarnings)
{
    if (!InResolution.mbFound)
    {
        std::cout << "Section not found in " << InDocPath << ": " << InResolution.mSectionQuery << "\n";
    }
    else
    {
        std::cout << "Section " << InResolution.mSectionId << " (" << InResolution.mSectionHeading << ") "
                  << "lines " << InResolution.mStartLine << "-" << InResolution.mEndLine << "\n";
    }
    PrintTextWarnings(InWarnings);
    return 0;
}
...
int RunSectionSchemaText(const std::string& InType, const std::vector<SectionSchemaEntry>& InEntries)
{
    std::cout << "Section schema: " << InType << "\n";
    std::cout << "Count: " << InEntries.size() << "\n";
    for (const SectionSchemaEntry& Entry : InEntries)
    {
        std::cout << (Entry.mbRequired ? "[required] " : "[optional] ") << Entry.mSectionId << "\n";
    }
    return 0;
}


int RunSectionListText(const std::vector<SectionCount>& InCounts, const bool InShowCount)
{
...
int RunSectionListDocText(const std::string& InDocPath, const std::vector<HeadingRecord>& InHeadings)
{
    std::cout << "Doc: " << InDocPath << "\n";
    std::cout << "Sections: " << InHeadings.size() << "\n";
    for (const HeadingRecord& Heading : InHeadings)
    {
        std::cout << "H" << Heading.mLevel << " L" << Heading.mLine << ": " << Heading.mSectionId << " (" << Heading.mText << ")\n";
    }
    return 0;
}


int RunExcerptText(const std::string& InDocPath, const SectionResolution& InResolution, const int InExcerptStartLine, const std::vector<std::string>& InExcerptLines, const std::vector<std::string>& InWarnings)
{
    ...
    for (size_t LineIndex = 0; LineIndex < InExcerptLines.size(); ++LineIndex)
    {
        ...
        std::cout << std::setw(6) << LineNumber << " | " << InExcerptLines[LineIndex] << "\n";
    }
    PrintTextWarnings(InWarnings);
    return 0;
}
...
int RunTableListText(const std::string& InDocPath, const std::vector<MarkdownTableRecord>& InTables, const std::vector<std::string>& InWarnings)
{
    std::cout << "Table count in " << InDocPath << ": " << InTables.size() << "\n";
    for (const MarkdownTableRecord& Table : InTables)
    {
        std::cout << "- table_id=" << Table.mTableId << " section=" << (Table.mSectionId.empty() ? "-" : Table.mSectionId) << " lines=" << Table.mStartLine << "-" << Table.mEndLine << " columns=" << Table.mHeaders.size() << " rows=" << Table.mRows.size()
                  << "\n";
    }
    PrintTextWarnings(InWarnings);
    return 0;
...
int RunTableGetText(const std::string& InDocPath, const MarkdownTableRecord& InTable, const std::vector<std::string>& InWarnings)
{
...
int RunGraphText(const std::string& InTopicKey, const int InDepth, const std::vector<GraphNode>& InNodes, const std::vector<GraphEdge>& InEdges, const std::vector<std::string>& InWarnings)
{
    std::cout << "Graph topic=" << InTopicKey << " depth=" << InDepth << " nodes=" << InNodes.size() << " edges=" << InEdges.size() << "\n";
    ...
    for (size_t Index = 0; Index < InNodes.size() && Index < MaxNodeRows; ++Index)
    {
        ...
        std::cout << "NODE " << Node.mId << " (" << Node.mType << "): " << Node.mPath << "\n";
    }
    if (InNodes.size() > MaxNodeRows)
    {
        std::cout << "... " << (InNodes.size() - MaxNodeRows) << " more nodes omitted\n";
    }

    for (size_t Index = 0; Index < InEdges.size() && Index < MaxEdgeRows; ++Index)
    {
        const GraphEdge& Edge = InEdges[Index];
        std::cout << "EDGE " << Edge.mFromNodeId << " -> " << Edge.mToNodeId << " (" << Edge.mKind << ", depth=" << Edge.mDepth << ")\n";
    }
    if (InEdges.size() > MaxEdgeRows)
    {
        std::cout << "... " << (InEdges.size() - MaxEdgeRows) << " more edges omitted\n";
    }
...
int RunDiagnoseDriftText(const bool InOk, const std::vector<DriftItem>& InDrifts, const std::vector<std::string>& InWarnings)
{
    std::cout << "Diagnose drift: " << (InOk ? "ok" : "drift_detected") << " (" << InDrifts.size() << " items)\n";
...
int RunTimelineText(const std::string& InTopicKey, const std::string& InSince, const std::vector<TimelineItem>& InItems, const std::vector<std::string>& InWarnings)
{
    std::cout << "Timeline topic=" << InTopicKey << " since=" << (InSince.empty() ? "-" : InSince) << " entries=" << InItems.size() << "\n";
    for (const TimelineItem& Item : InItems)
    {
        std::cout << "- " << (Item.mDate.empty() ? "unknown-date" : Item.mDate) << " [" << Item.mDocClass;
...
int RunPhaseText(const std::string& InTopicKey, const std::string& InPlanPath, const std::string& InStatusFilter, const std::vector<PhaseItem>& InItems, const std::vector<std::string>& InWarnings)
{
    std::cout << "Phase list topic=" << InTopicKey << " status=" << InStatusFilter << " count=" << InItems.size() << " plan=" << InPlanPath << "\n";
...
int RunBlockersText(const std::string& InStatusFilter, const std::vector<BlockerItem>& InItems, const std::vector<std::string>& InWarnings)
{
    std::cout << "Blockers status=" << InStatusFilter << " count=" << InItems.size() << "\n";
    for (const BlockerItem& Item : InItems)
    {
        std::cout << "- " << Item.mTopicKey << " [" << Item.mKind << "/" << Item.mStatus << "]";
        if (!Item.mPhaseKey.empty())
        {
...
int RunCacheInfoText(const CacheInfoResult& InResult)
{
    ...
    {
        std::cout << "Size: " << FormatBytesHuman(InResult.mCacheSizeBytes) << " (" << InResult.mCacheSizeBytes << " bytes)\n";
        std::cout << "Entries: " << InResult.mCacheEntryCount << "\n";
    }
    std::cout << "Current repo cache: " << InResult.mCurrentRepoCachePath << "\n";
    std::cout << "Current repo cached: " << (InResult.mbCurrentRepoCacheExists ? "yes" : "no") << "\n";
    std::cout << "Config dir override: " << (InResult.mConfigCacheDir.empty() ? "(default)" : InResult.mConfigCacheDir) << "\n";
...
int RunCacheClearText(const CacheClearResult& InResult)
{
    ...
    {
        std::cout << "Cleared " << InResult.mEntriesRemoved << " entries, freed " << FormatBytesHuman(InResult.mBytesFreed) << ".\n";
    }
    PrintTextWarnings(InResult.mWarnings);
...
int RunCacheConfigText(const CacheConfigResult& InResult)
{
    ...
    std::cout << "  dir     = " << (InResult.mDir.empty() ? "(default)" : InResult.mDir) << "\n";
...
int RunPhaseListAllText(const std::string& InStatusFilter, const std::vector<PhaseListAllEntry>& InEntries, const std::vector<std::string>& InWarnings)
{
    ...
    std::cout << "Phase list (all) status=" << InStatusFilter << " plans=" << InEntries.size() << " phases=" << TotalPhases << "\n";
    ...
    for (size_t PlanIndex = 0; PlanIndex < InEntries.size(); ++PlanIndex)
    {
        ...
        std::cout << Entry.mTopicKey << " status=" << Entry.mPlanStatus << " plan=" << Entry.mPlanPath << " phases=" << Entry.mPhases.size() << "\n";
...
int RunSectionContentText(const std::string& InDocPath, const SectionResolution& InResolution, const int InLineCharLimit, const int InContentStartLine, const std::vector<std::string>& InContentLines, const std::vector<std::string>& InWarnings)
{
    std::cout << "Section content " << InDocPath << " [" << InResolution.mSectionId << "]\n";
