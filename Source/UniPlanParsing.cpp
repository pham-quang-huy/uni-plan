...
StatusInference InferDocumentStatus(const EDocumentKind InKind,
  ...
  if (!TryReadFileLines(InAbsolutePath, Lines, ReadError)) {
    AddWarning(OutWarnings, "Status inference skipped for '" +
                                InAbsolutePath.string() + "': " + ReadError);
    return {};
  }
  ...
  for (const std::string &Line : Lines) {
    ...
    if (!Trimmed.empty() && Trimmed.front() == '#') {
      ...
      InImplementationPhases =
          (LowerHeading.find("implementation_phases") != std::string::npos);
      InExecutionLanes =
    ...
    // Plan implementation_phases and impl phase_tracking no longer carry Status
    // columns. Phase status is derived from playbook execution_lanes (single
    // source of truth). Skip these sections for status inference.
  ...
  if (Result.mNormalized != "unknown") {
    Result.mRaw =
        Counters.mFirstRaw.empty() ? Result.mNormalized : Counters.mFirstRaw;
  }
  return Result;
}
...
bool TryClassifyCoreDocument(const std::string &InRelativePath,
  ...
  static const std::regex PlanNameRegex(R"(^([A-Za-z0-9]+)\.Plan\.md$)");
  static const std::regex PlaybookNameRegex(
      R"(^([A-Za-z0-9]+)\.([^.]+)\.Playbook\.md$)");
...
BuildTopicPairs(const std::vector<DocumentRecord> &InPlans,
  ...
  for (const DocumentRecord &Record : InPlans) {
    ...
    if (!Aggregate.mPlanPath.empty() && Aggregate.mPlanPath != Record.mPath) {
      AddWarning(OutWarnings, "Duplicate plan for topic '" + Record.mTopicKey +
                                  "': '" + Aggregate.mPlanPath + "', '" +
                                  Record.mPath + "'.");
    }
  ...
  for (const DocumentRecord &Record : InPlaybooks) {
    ...
    if (ExistingPhase != Aggregate.mPlaybookPhaseToPath.end() &&
      ...
      AddWarning(OutWarnings,
                 "Duplicate playbook for topic '" + Record.mTopicKey +
                     "' and phase '" + Record.mPhaseKey + "': '" +
                     ExistingPhase->second + "', '" + Record.mPath + "'.");
    }
  ...
  for (auto &Entry : Topics) {
    ...
    AddStatusCandidate(PairStatusCounters, PairRecord.mImplementationStatus);
...
void AppendSidecarIntegrityWarnings(
  ...
  for (const SidecarRecord &Sidecar : InSidecars) {
    ...
    const auto ExistingSidecar = SidecarIdentityToPath.find(SidecarIdentity);
    if (ExistingSidecar != SidecarIdentityToPath.end() &&
        ExistingSidecar->second != Sidecar.mPath) {
      AddWarning(OutWarnings,
                 "Duplicate sidecar for '" + Sidecar.mOwnerKind + "' topic '" +
                     Sidecar.mTopicKey + "' phase '" + Sidecar.mPhaseKey +
                     "' doc-kind '" + Sidecar.mDocKind + "': '" +
                     ExistingSidecar->second + "', '" + Sidecar.mPath + "'.");
    }
    ...
    if (Sidecar.mOwnerKind == "Playbook") {
      ...
      if (PlaybookKeys.count(PlaybookIdentity) == 0) {
        AddWarning(
            OutWarnings,
            "Orphan playbook sidecar without playbook owner for topic '" +
                Sidecar.mTopicKey + "' phase '" + Sidecar.mPhaseKey + "': '" +
                Sidecar.mPath + "'.");
      }
      continue;
...
fs::path NormalizeRepoRootPath(const std::string &InRepoRoot) {
  ...
  if (Error) {
    throw std::runtime_error(
        "Failed to resolve repository root to absolute path '" + InRepoRoot +
        "': " + Error.message());
  }
...
Inventory BuildInventoryFresh(const std::string &InRepoRoot) {
  ...
  if (!fs::exists(RepoRoot, Error)) {
    if (Error) {
      throw std::runtime_error("Failed to check repository root existence '" +
  ...
  if (Error) {
    throw std::runtime_error("Failed to initialize repository traversal at '" +
                             RepoRoot.string() + "': " + Error.message());
  }
  ...
  while (Iterator != EndIterator) {
    ...
    const auto AdvanceIterator = [&Iterator, &EndIterator, &Result,
                                  &AbsolutePath]() {
      std::error_code AdvanceError;
      Iterator.increment(AdvanceError);
      if (AdvanceError) {
        AddWarning(Result.mWarnings, "Traversal advance failure after path '" +
                                         AbsolutePath.string() +
    ...
    if (TryClassifyCoreDocument(Relative, Filename, CoreRecord)) {
      const StatusInference Status =
          InferDocumentStatus(CoreRecord.mKind, AbsolutePath, Result.mWarnings);
...
EnumerateMarkdownDocuments(const fs::path &InRepoRoot,
  ...
  fs::recursive_directory_iterator EndIterator;
  if (Error) {
    throw std::runtime_error("Failed to initialize markdown traversal at '" +
  ...
  while (Iterator != EndIterator) {
    const fs::directory_entry Entry = *Iterator;
    const fs::path AbsolutePath = Entry.path();
    const auto AdvanceIterator = [&Iterator, &EndIterator, &OutWarnings,
                                  &AbsolutePath]() {
      std::error_code AdvanceError;
      Iterator.increment(AdvanceError);
      if (AdvanceError) {
        AddWarning(OutWarnings, "Traversal advance failure after path '" +
                                    AbsolutePath.string() +
                                    "': " + AdvanceError.message());
        Iterator = EndIterator;
      }
    ...
    const bool IsRegularFile = Entry.is_regular_file(PathTypeError);
    if (PathTypeError) {
      AddWarning(OutWarnings, "Skipping path due to file-type read failure '" +
                                  AbsolutePath.string() +
...
ParseMarkdownTables(const std::vector<std::string> &InLines,
  ...
  for (size_t Index = 0; Index < InLines.size(); ++Index) {
    ...
    while (RowIndex < InLines.size()) {
      ...
      const std::vector<std::string> RowCells = SplitMarkdownTableRow(RowLine);
...
std::vector<EvidenceEntry> ParseEvidenceEntriesFromFile(
  ...
  if (!TryReadFileLines(InAbsolutePath, Lines, ReadError)) {
    AddWarning(OutWarnings,
               "Failed to read sidecar '" + InRelativePath + "': " + ReadError);
    return {};
  }
  ...
  if (Tables.empty()) {
    AddWarning(OutWarnings,
               "No markdown tables found in sidecar '" + InRelativePath + "'.");
    return {};
  }
  ...
  for (const MarkdownTableRecord &Table : Tables) {
    ...
    for (size_t RowIndex = 0; RowIndex < Table.mRows.size(); ++RowIndex) {
      ...
      for (size_t HeaderIndex = 0; HeaderIndex < Table.mHeaders.size();
           ++HeaderIndex) {
        const std::string Value = (HeaderIndex < Table.mRows[RowIndex].size())
...
std::map<std::string, std::string> BuildFieldMap(
    const std::vector<std::pair<std::string, std::string>> &InFields) {
...
ExtractReferencedMarkdownPaths(const fs::path &InRepoRoot,
  ...
  for (std::sregex_iterator
    ...
    try {
      Result.insert(ToGenericPath(fs::relative(CandidateAbsolute, InRepoRoot)));
    } catch (const fs::filesystem_error &) {
      continue;
    }
  }
...
TryParseSectionSchemaFromFile(const fs::path &InSchemaPath) {
  ...
  while (Cursor < Lines.size()) {
    ...
    if (OrderCol >= 0 && static_cast<int>(Cells.size()) > OrderCol) {
      try {
        Entry.mOrder = std::stoi(Trim(Cells[static_cast<size_t>(OrderCol)]));
      } catch (...) {
