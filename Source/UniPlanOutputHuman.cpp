...
struct HumanTable {
  ...
  void AddRow(const std::vector<std::string> &InRow) { mRows.push_back(InRow); }
...
int RunListHuman(const Inventory &InInventory, const std::string &InKind,
  ...
  if (InKind == "pair") {
    const std::vector<const TopicPairRecord *> FilteredPairs =
        FilterPairsByStatus(InInventory.mPairs, InStatusFilter);
    ...
    std::cout << " ("
              << Colorize(kColorOrange, std::to_string(FilteredPairs.size()))
              << " topics)\n\n";
    HumanTable Table;
    Table.mHeaders = {"TopicKey",   "PairState",  "OverallStatus",
                      "PlanStatus", "ImplStatus", "Playbooks"};
    ...
    if (bFilterActive && FilteredPairs.empty()) {
      std::cout << Colorize(kColorYellow, "No pairs found with status '" +
                                              InStatusFilter + "'.")
                << "\n";
    } else {
      Table.Print();
  ...
  std::cout << " ("
            << Colorize(kColorOrange, std::to_string(FilteredRecords.size()))
            << " docs)\n\n";
  if (bFilterActive && FilteredRecords.empty()) {
    std::cout << Colorize(kColorYellow, "No " + InKind +
                                            " found with status '" +
                                            InStatusFilter + "'.")
              << "\n";
    return 0;
  ...
  for (const DocumentRecord *RecordPtr : FilteredRecords) {
    ...
    if (InKind == "playbook") {
      Table.AddRow({Colorize(kColorOrange, Record.mTopicKey), Record.mPhaseKey,
                    ColorizeStatus(StatusValue), ColorizeStatus(PairState),
                    Colorize(kColorDim, Record.mPath)});
    } else {
      Table.AddRow({Colorize(kColorOrange, Record.mTopicKey),
                    ColorizeStatus(StatusValue), ColorizeStatus(PairState),
                    Colorize(kColorDim, Record.mPath)});
    }
  }
...
int RunLintHuman(const LintResult &InResult) {
  ...
  std::cout << "\n"
            << ColorBold("Lint warnings: ")
            << Colorize(kColorOrange, std::to_string(InResult.mWarningCount))
            << "\n";
  return 0;
}

int RunInventoryHuman(const InventoryResult &InResult) {
  std::cout << ColorBold("Inventory") << " ("
            << Colorize(kColorOrange, std::to_string(InResult.mItems.size()))
            << " docs)\n\n";
  HumanTable Table;
  Table.mHeaders = {"Path", "Lines", "LastCommit"};
  for (const InventoryItem &Item : InResult.mItems) {
    Table.AddRow({Colorize(kColorDim, Item.mPath),
                  Colorize(kColorOrange, std::to_string(Item.mLineCount)),
                  Item.mLastCommit});
  }
  Table.Print();
...
int RunOrphanCheckHuman(const OrphanCheckResult &InResult) {
  std::cout << ColorBold("Orphan docs: ")
            << Colorize(kColorOrange, std::to_string(InResult.mOrphans.size()))
            << "\n";
  for (const std::string &Orphan : InResult.mOrphans) {
    std::cout << Colorize(kColorRed, "ORPHAN") << " "
              << Colorize(kColorDim, Orphan) << "\n";
  }
  PrintHumanWarnings(InResult.mWarnings);
  return 0;
}
...
int RunArtifactsHuman(const Inventory &InInventory,
  ...
  std::cout << ColorBold("PairState: ")
            << ColorizeStatus(ResolvePairStateForTopic(InInventory, InTopicKey))
            << "\n";
  if ((InKind == "all" || InKind == "plan") && Plan != nullptr) {
    std::cout << ColorBold("Plan: ") << Colorize(kColorDim, Plan->mPath) << " ("
              << ColorizeStatus(GetDisplayStatus(Plan->mStatus)) << ")\n";
  }
  if ((InKind == "all" || InKind == "implementation") &&
      Implementation != nullptr) {
    std::cout << ColorBold("Implementation: ")
              << Colorize(kColorDim, Implementation->mPath) << " ("
              << ColorizeStatus(GetDisplayStatus(Implementation->mStatus))
              << ")\n";
  }
  if (InKind == "all" || InKind == "playbook") {
    std::cout << ColorBold("Playbooks: ")
              << Colorize(kColorOrange, std::to_string(Playbooks.size()))
              << "\n";
    for (const DocumentRecord &Playbook : Playbooks) {
      std::cout << "  - [" << Colorize(kColorOrange, Playbook.mPhaseKey) << "] "
                << Colorize(kColorDim, Playbook.mPath) << " ("
                << ColorizeStatus(GetDisplayStatus(Playbook.mStatus)) << ")\n";
    }
  }
  if (InKind == "all" || InKind == "sidecar") {
    std::cout << ColorBold("Sidecars: ")
              << Colorize(kColorOrange, std::to_string(Sidecars.size()))
              << "\n";
    for (const SidecarRecord &Sidecar : Sidecars) {
      std::cout << "  - " << Sidecar.mOwnerKind << " "
                << (Sidecar.mPhaseKey.empty() ? "-" : Sidecar.mPhaseKey) << " "
                << Sidecar.mDocKind << ": "
                << Colorize(kColorDim, Sidecar.mPath) << "\n";
    }
  }
...
int RunEvidenceHuman(const std::string &InLabel, const std::string &InTopicKey,
  ...
  std::cout << ColorBold(InLabel)
            << " topic=" << Colorize(kColorOrange, InTopicKey)
            << " doc_class=" << InDocClass << " entries="
            << Colorize(kColorOrange, std::to_string(InEntries.size()))
            << "\n\n";
  HumanTable Table;
  Table.mHeaders = {"Source", "Phase", "TableId", "Row", "Fields"};
  for (const EvidenceEntry &Entry : InEntries) {
    std::ostringstream Fields;
    for (size_t FieldIndex = 0; FieldIndex < Entry.mFields.size();
         ++FieldIndex) {
      if (FieldIndex > 0) {
        Fields << ", ";
      }
      Fields << Entry.mFields[FieldIndex].first << "="
             << Entry.mFields[FieldIndex].second;
    }
    Table.AddRow({Colorize(kColorDim, Entry.mSourcePath),
                  Entry.mPhaseKey.empty() ? "-" : Entry.mPhaseKey,
                  std::to_string(Entry.mTableId),
                  std::to_string(Entry.mRowIndex), Fields.str()});
  }
  Table.Print();
...
int RunSchemaHuman(const std::string &InType,
  ...
  std::cout << ColorBold("Schema type: ") << Colorize(kColorOrange, InType)
            << "\n";
  std::cout << ColorBold("Fields: ")
            << Colorize(kColorOrange, std::to_string(InFields.size()))
            << "\n\n";
  HumanTable Table;
  Table.mHeaders = {"Section", "Property", "Value"};
  for (const SchemaField &Field : InFields) {
    Table.AddRow({Colorize(kColorDim, Field.mSectionId),
                  Colorize(kColorOrange, Field.mProperty), Field.mValue});
  }
  Table.Print();
  if (!InExamples.empty()) {
    std::cout << "\n" << ColorBold("Examples:") << "\n";
...
int RunRulesHuman(const std::vector<RuleEntry> &InRules,
  ...
  std::cout << ColorBold("Rules: ")
            << Colorize(kColorOrange, std::to_string(InRules.size()))
            << " (resolved="
            << Colorize(kColorGreen, std::to_string(ResolvedCount))
            << ", unresolved="
            << Colorize(kColorYellow,
                        std::to_string(static_cast<int>(InRules.size()) -
                                       ResolvedCount))
            << ")\n\n";
  HumanTable Table;
  Table.mHeaders = {"Id", "Description", "Source", "Resolved"};
  for (const RuleEntry &Rule : InRules) {
    Table.AddRow({Colorize(kColorOrange, Rule.mId), Rule.mDescription,
                  Colorize(kColorDim, Rule.mSource),
                  Rule.mbSourceResolved ? Colorize(kColorGreen, "yes")
                                        : Colorize(kColorYellow, "no")});
  }
  Table.Print();
  PrintHumanWarnings(InWarnings);
  return 0;
...
int RunValidateHuman(const bool InStrict, const bool InOk,
                     ...
                     const std::vector<std::string> &InWarnings) {
  std::cout << ColorBold("Validate: ")
            << (InOk ? Colorize(kColorGreen, "ok")
                     : Colorize(kColorRed, "failed"))
  ...
  for (const ValidateCheck &Check : InChecks) {
    ...
    Table.AddRow({Colorize(kColorOrange, Check.mId),
                  Check.mbOk ? Colorize(kColorGreen, "ok")
                             : Colorize(kColorRed, "failed"),
                  Check.mbCritical ? Colorize(kColorYellow, "yes") : "no",
                  Detail});
  }
  Table.Print();
  for (const ValidateCheck &Check : InChecks) {
...
int RunSectionResolveHuman(const std::string &InDocPath,
  ...
  if (!InResolution.mbFound) {
    std::cout << "Section " << Colorize(kColorRed, "not found") << " in "
              << Colorize(kColorDim, InDocPath) << ": "
              << Colorize(kColorOrange, InResolution.mSectionQuery) << "\n";
  } else {
    std::cout << "Section " << Colorize(kColorOrange, InResolution.mSectionId)
              << " (" << InResolution.mSectionHeading << ") "
              << Colorize(kColorGreen,
                          "lines " + std::to_string(InResolution.mStartLine) +
                              "-" + std::to_string(InResolution.mEndLine))
              << "\n";
  }
  PrintHumanWarnings(InWarnings);
  return 0;
...
int RunSectionSchemaHuman(const std::string &InType,
  ...
  std::cout << ColorBold("Section schema: " + InType) << " ("
            << Colorize(kColorOrange, std::to_string(InEntries.size()))
            << " sections)\n\n";
  HumanTable Table;
  Table.mHeaders = {"Order", "Section ID", "Required"};
  for (const SectionSchemaEntry &Entry : InEntries) {
    const std::string RequiredLabel = Entry.mbRequired
                                          ? Colorize(kColorGreen, "required")
                                          : Colorize(kColorDim, "optional");
    Table.AddRow({std::to_string(Entry.mOrder),
                  Colorize(kColorOrange, Entry.mSectionId), RequiredLabel});
  }
  Table.Print();
  return 0;
}
...
int RunSectionListHuman(const std::vector<SectionCount> &InCounts,
  ...
  std::cout << ColorBold("Section list") << " ("
            << Colorize(kColorOrange, std::to_string(InCounts.size()))
            << " unique headings)\n\n";
  if (InShowCount) {
    HumanTable Table;
    Table.mHeaders = {"Heading", "Section ID", "Count"};
    for (const SectionCount &Item : InCounts) {
      Table.AddRow({Colorize(kColorGreen, Item.mHeading),
                    Colorize(kColorOrange, Item.mSectionId),
                    Colorize(kColorGreen, std::to_string(Item.mCount))});
    }
    Table.Print();
  } else {
    for (const SectionCount &Item : InCounts) {
      std::cout << Colorize(kColorGreen, Item.mHeading) << " "
                << Colorize(kColorDim, "[" + Item.mSectionId + "]") << "\n";
    }
  }
  return 0;
}
...
int RunSectionListDocHuman(const std::string &InDocPath,
  ...
  std::cout << ColorBold("Sections") << " " << Colorize(kColorDim, InDocPath)
            << " (" << Colorize(kColorOrange, std::to_string(InHeadings.size()))
            << " headings)\n\n";
  ...
  // For each heading, determine its children by scanning forward until a
  // heading at the same or higher level is found. Use a stack to track
  // connector state.
  for (size_t Index = 0; Index < InHeadings.size(); ++Index) {
    ...
    // A sibling is the next heading at the same level with no intervening
    // heading at a lower (parent) level.
    ...
    // For levels 2..Level-1, determine if the ancestor at that level has more
    // siblings.
    std::string Prefix;
    for (int PrefixLevel = 2; PrefixLevel < Level; ++PrefixLevel) {
      // Check if there's a future heading at PrefixLevel that is an ancestor
      // connector.
      bool bAncestorHasMore = false;
      for (size_t FutureIndex = Index + 1; FutureIndex < InHeadings.size();
           ++FutureIndex) {
    ...
    if (Level > 1) {
      Prefix += bIsLastChild ? "\xe2\x94\x94\xe2\x94\x80\xe2\x94\x80 "
                             : "\xe2\x94\x9c\xe2\x94\x80\xe2\x94\x80 ";
    }

    const std::string Label =
        Colorize(kColorGreen, Heading.mText) +
        Colorize(kColorDim, " [" + Heading.mSectionId + "]") +
        Colorize(kColorDim, " (H" + std::to_string(Level) + ", L" +
...
int RunExcerptHuman(const std::string &InDocPath,
                    ...
                    const std::vector<std::string> &InWarnings) {
  std::cout << ColorBold("Excerpt ") << Colorize(kColorDim, InDocPath) << " ["
            << Colorize(kColorOrange, InResolution.mSectionId) << "]\n";
...
int RunSectionContentHuman(const std::string &InDocPath,
                           ...
                           const std::vector<std::string> &InWarnings) {
  std::cout << ColorBold("Section content ") << "["
            << Colorize(kColorOrange, InResolution.mSectionId) << "] "
            << Colorize(kColorDim, InDocPath) << "\n\n";

  // Build a set of line ranges that are tables so we can render them via
  // HumanTable. Each entry maps a 1-based line number to a table index.
  ...
  for (size_t LineIndex = 0; LineIndex < InContentLines.size(); ++LineIndex) {
    ...
    if (TableCoveredLines.count(LineNumber) &&
        !TableStartLines.count(LineNumber)) {
      continue;
    }
    ...
    if (TableIt != TableStartLines.end()) {
      ...
      if (InLineCharLimit > 0) {
        ...
        for (const std::string &Header : Table.mHeaders) {
          TruncatedHeaders.push_back(
              TruncateForDisplay(Header, static_cast<size_t>(InLineCharLimit)));
        }
        ...
        for (const std::vector<std::string> &Row : Table.mRows) {
          ...
          for (const std::string &Cell : Row) {
            TruncatedRow.push_back(
                TruncateForDisplay(Cell, static_cast<size_t>(InLineCharLimit)));
          }
...
int RunTableListHuman(const std::string &InDocPath,
                      const std::vector<MarkdownTableRecord> &InTables,
                      const std::vector<std::string> &InWarnings) {
  std::cout << ColorBold("Tables in ") << Colorize(kColorDim, InDocPath) << " ("
            << Colorize(kColorOrange, std::to_string(InTables.size()))
            << ")\n\n";
  HumanTable Table;
  Table.mHeaders = {"Id", "Section", "Lines", "Cols", "Rows"};
  for (const MarkdownTableRecord &Record : InTables) {
    Table.AddRow({Colorize(kColorOrange, std::to_string(Record.mTableId)),
                  Record.mSectionId.empty() ? "-" : Record.mSectionId,
                  std::to_string(Record.mStartLine) + "-" +
                      std::to_string(Record.mEndLine),
...
int RunTableGetHuman(const std::string &InDocPath,
                     ...
                     const std::vector<std::string> &InWarnings) {
  std::cout << ColorBold("Table " + std::to_string(InTable.mTableId)) << " in "
            << Colorize(kColorDim, InDocPath) << "\n\n";
  HumanTable Table;
...
int RunGraphHuman(const std::string &InTopicKey, const int InDepth,
                  ...
                  const std::vector<std::string> &InWarnings) {
  std::cout << ColorBold("Graph")
            << " topic=" << Colorize(kColorOrange, InTopicKey)
            << " depth=" << InDepth << " nodes="
            << Colorize(kColorOrange, std::to_string(InNodes.size()))
            << " edges="
            << Colorize(kColorOrange, std::to_string(InEdges.size())) << "\n\n";
  ...
  const size_t MaxEdgeRows = 180;
  {
    ...
    for (size_t Index = 0; Index < InNodes.size() && Index < MaxNodeRows;
      ...
      NodeTable.AddRow({Colorize(kColorOrange, Node.mId),
                        Colorize(kColorGreen, Node.mType),
                        Colorize(kColorDim, Node.mPath)});
    }
    NodeTable.Print();
    if (InNodes.size() > MaxNodeRows) {
      std::cout << Colorize(kColorDim,
                            "... " +
                                std::to_string(InNodes.size() - MaxNodeRows) +
                                " more nodes omitted")
                << "\n";
    }
  }
  ...
  std::cout << "\n";
  {
    ...
    for (size_t Index = 0; Index < InEdges.size() && Index < MaxEdgeRows;
      ...
      EdgeTable.AddRow({Colorize(kColorOrange, Edge.mFromNodeId),
                        Edge.mToNodeId, Colorize(kColorGreen, Edge.mKind),
                        std::to_string(Edge.mDepth)});
    }
    EdgeTable.Print();
    if (InEdges.size() > MaxEdgeRows) {
      std::cout << Colorize(kColorDim,
                            "... " +
                                std::to_string(InEdges.size() - MaxEdgeRows) +
                                " more edges omitted")
                << "\n";
    }
  }
...
int RunDiagnoseDriftHuman(const bool InOk,
                          ...
                          const std::vector<std::string> &InWarnings) {
  std::cout << ColorBold("Diagnose drift: ")
            << (InOk ? Colorize(kColorGreen, "ok")
                     : Colorize(kColorRed, "drift_detected"))
            << " (" << Colorize(kColorOrange, std::to_string(InDrifts.size()))
            << " items)\n\n";
  if (!InDrifts.empty()) {
    HumanTable Table;
    ...
    for (const DriftItem &Drift : InDrifts) {
      const char *SeverityColor =
          (Drift.mSeverity == "error") ? kColorRed : kColorYellow;
      Table.AddRow({Colorize(SeverityColor, Drift.mSeverity), Drift.mId,
                    Drift.mTopicKey.empty()
                        ? "-"
                        : Colorize(kColorOrange, Drift.mTopicKey),
                    Drift.mMessage});
    }
    Table.Print();
  }
...
int RunTimelineHuman(const std::string &InTopicKey, const std::string &InSince,
                     ...
                     const std::vector<std::string> &InWarnings) {
  std::cout << ColorBold("Timeline")
            << " topic=" << Colorize(kColorOrange, InTopicKey)
            << " since=" << (InSince.empty() ? "-" : InSince) << " entries="
            << Colorize(kColorOrange, std::to_string(InItems.size())) << "\n\n";
  if (!InItems.empty()) {
    HumanTable Table;
    Table.mHeaders = {"Date", "Class", "Phase", "Update", "Source"};
    for (const TimelineItem &Item : InItems) {
      Table.AddRow({Colorize(kColorOrange, GetDisplayStatus(Item.mDate)),
                    Colorize(kColorGreen, Item.mDocClass),
                    Item.mPhaseKey.empty() ? "-" : Item.mPhaseKey, Item.mUpdate,
                    Colorize(kColorDim, Item.mSourcePath)});
    }
    Table.Print();
  }
  PrintHumanWarnings(InWarnings);
...
int RunPhaseHuman(const std::string &InTopicKey, const std::string &InPlanPath,
                  ...
                  const std::vector<std::string> &InWarnings) {
  std::cout << ColorBold("Phase list")
            << " topic=" << Colorize(kColorOrange, InTopicKey)
            << " status=" << InStatusFilter << " count="
            << Colorize(kColorOrange, std::to_string(InItems.size()))
            << " plan=" << Colorize(kColorDim, InPlanPath) << "\n\n";
  if (!InItems.empty()) {
    HumanTable Table;
    Table.mHeaders = {"Phase", "Status", "Description", "Next Action",
                      "Playbook"};
    for (const PhaseItem &Item : InItems) {
      const std::string DescDisplay =
          Item.mDescription.empty() ? std::string("-")
                                    : TruncateForDisplay(Item.mDescription, 40);
      const std::string NextDisplay =
          Item.mNextAction.empty() ? std::string("-")
                                   : TruncateForDisplay(Item.mNextAction, 40);
      Table.AddRow(
          {Colorize(kColorOrange, Item.mPhaseKey), ColorizeStatus(Item.mStatus),
           Colorize(kColorDim, DescDisplay), Colorize(kColorDim, NextDisplay),
           Item.mPlaybookPath.empty()
               ? "-"
               : Colorize(kColorDim, Item.mPlaybookPath)});
    }
    Table.Print();
  }
  PrintHumanWarnings(InWarnings);
...
int RunPhaseListAllHuman(const std::string &InStatusFilter,
  ...
  std::cout << ColorBold("Phase List (all)") << " status=" << InStatusFilter
            << " plans="
            << Colorize(kColorOrange, std::to_string(InEntries.size()))
            << " phases=" << Colorize(kColorOrange, std::to_string(TotalPhases))
            << "\n\n";
  ...
  for (size_t PlanIndex = 0; PlanIndex < InEntries.size(); ++PlanIndex) {
    ...
    std::cout << Colorize(kColorOrange, Entry.mTopicKey) << "  "
              << ColorizeStatus(Entry.mPlanStatus) << "  "
              << Colorize(kColorDim, Entry.mPlanPath) << "\n";
    for (size_t PhaseIndex = 0; PhaseIndex < Entry.mPhases.size();
         ++PhaseIndex) {
      const PhaseItem &Item = Entry.mPhases[PhaseIndex];
      const bool bIsLast = (PhaseIndex + 1 == Entry.mPhases.size());
      const char *Connector = bIsLast ? "\xe2\x94\x94\xe2\x94\x80\xe2\x94\x80 "
                                      : "\xe2\x94\x9c\xe2\x94\x80\xe2\x94\x80 ";
      const std::string DescDisplay =
          Item.mDescription.empty()
              ? std::string()
              : "  " + TruncateForDisplay(Item.mDescription, 50);
      std::cout << "  " << Connector << Colorize(kColorOrange, Item.mPhaseKey)
                << "  " << ColorizeStatus(Item.mStatus)
                << Colorize(kColorDim, DescDisplay) << "\n";
    }
  }
  PrintHumanWarnings(InWarnings);
  return 0;
...
int RunBlockersHuman(const std::string &InStatusFilter,
                     ...
                     const std::vector<std::string> &InWarnings) {
  std::cout << ColorBold("Blockers") << " status=" << InStatusFilter
            << " count="
            << Colorize(kColorOrange, std::to_string(InItems.size())) << "\n\n";
  if (!InItems.empty()) {
    HumanTable Table;
    ...
    for (const BlockerItem &Item : InItems) {
      Table.AddRow({Colorize(kColorOrange, Item.mTopicKey), Item.mKind,
                    ColorizeStatus(Item.mStatus),
                    Item.mPhaseKey.empty() ? "-" : Item.mPhaseKey,
                    Item.mPriority.empty()
                        ? "-"
                        : Colorize(kColorYellow, Item.mPriority),
                    Item.mAction});
    }
    Table.Print();
  }
...
int RunCacheInfoHuman(const CacheInfoResult &InResult) {
  ...
  Table.AddRow({"Exists", InResult.mbCacheExists ? Colorize(kColorGreen, "yes")
                                                 : Colorize(kColorDim, "no")});
  if (InResult.mbCacheExists) {
    Table.AddRow(
        {"Size",
         Colorize(kColorOrange, FormatBytesHuman(InResult.mCacheSizeBytes))});
    Table.AddRow(
        {"Entries",
         Colorize(kColorOrange, std::to_string(InResult.mCacheEntryCount))});
  }
...
int RunCacheClearHuman(const CacheClearResult &InResult) {
  ...
  if (InResult.mEntriesRemoved == 0 && InResult.mBytesFreed == 0) {
    ...
    std::cout << "Cleared "
              ...
              << " entries, freed "
              << Colorize(kColorOrange, FormatBytesHuman(InResult.mBytesFreed))
              << ".\n";
  }
...
int RunCacheConfigHuman(const CacheConfigResult &InResult) {
  ...
  Table.AddRow({"enabled", InResult.mbEnabled
                               ? Colorize(kColorGreen, "true")
                               : Colorize(kColorYellow, "false")});
  Table.AddRow({"verbose", InResult.mbVerbose ? Colorize(kColorGreen, "true")
                                              : Colorize(kColorDim, "false")});
  Table.Print();
