...
DocWatchApp::DocWatchApp(const std::string& InRepoRoot, const DocConfig& InConfig) : mRepoRoot(InRepoRoot), mConfig(InConfig), mbUseCache(InConfig.mbCacheEnabled)
{
}
...
int DocWatchApp::Run()
{
    ...
    auto dashboard = Renderer([&] {
        ...
        if (mbActiveBlockFocused && mSelectedPlanIndex >= 0 && mSelectedPlanIndex < static_cast<int>(mSnapshot.mActivePlans.size()))
        {
            SelectedPlan = mSnapshot.mActivePlans[static_cast<size_t>(mSelectedPlanIndex)];
        }
        else if (!mbActiveBlockFocused && mSelectedNonActiveIndex >= 0 && mSelectedNonActiveIndex < static_cast<int>(mSnapshot.mNonActivePlans.size()))
        {
            SelectedPlan = mSnapshot.mNonActivePlans[static_cast<size_t>(mSelectedNonActiveIndex)];
        }
        ...
        auto leftRow3 = PanelActivePlans.Render(mSnapshot.mActivePlans, mSelectedPlanIndex);
        auto leftRow4 = PanelCompleted.Render(mSnapshot.mNonActivePlans, mSelectedNonActiveIndex);
        ...
        {
            for (int PhIdx = 0; PhIdx < static_cast<int>(SelectedPlan.mPhases.size()); ++PhIdx)
            {
                if (SelectedPlan.mPhases[static_cast<size_t>(PhIdx)].mStatus == "in_progress")
                {
        ...
        auto rightRow2 = PanelTaxonomy.Render(SelectedPlan, mSelectedPhaseIndex, mSelectedWaveIndex, mSelectedLaneIndex);

        // Resolve selected phase taxonomy for file manifest
        FWatchPhaseTaxonomy SelectedTaxonomy;
        if (mSelectedPhaseIndex >= 0 && mSelectedPhaseIndex < static_cast<int>(SelectedPlan.mPhases.size()))
        {
            const std::string& PhaseKey = SelectedPlan.mPhases[static_cast<size_t>(mSelectedPhaseIndex)].mPhaseKey;
        ...
        auto rightRow5 = PanelValidationFail.Render(mSnapshot.mValidation.mFailedCheckDetails);
        ...
        // ── Status bar ────────────────────────────────────────
        auto bar = PanelStatusBar.Render(kCliVersion, mSnapshot.mSnapshotAtUTC, mTickCount, mSnapshot.mPollDurationMs, mSnapshot.mInventory);

        // ── Title ─────────────────────────────────────────────
        const std::string Title = " FIE DOC WATCH  v" + std::string(kCliVersion) + "  \xe2\x97\x8f  " + mSnapshot.mSnapshotAtUTC + " ";
    ...
    dashboard = CatchEvent(dashboard, [this, &screen](Event InEvent) {
        ...
        if (InEvent == Event::Character('a'))
        {
            ...
            {
                mSelectedPlanIndex = (mSelectedPlanIndex + 1) % static_cast<int>(mSnapshot.mActivePlans.size());
            }
            return true;
        }
        if (InEvent == Event::Character('A'))
        {
            ...
            if (!mSnapshot.mActivePlans.empty())
            {
                mSelectedPlanIndex = (mSelectedPlanIndex - 1 + static_cast<int>(mSnapshot.mActivePlans.size())) % static_cast<int>(mSnapshot.mActivePlans.size());
            }
            return true;
        }
        if (InEvent == Event::Character('n'))
        {
            ...
            if (!mSnapshot.mNonActivePlans.empty())
            {
                ...
                mSelectedNonActiveIndex = (mSelectedNonActiveIndex + 1) % static_cast<int>(mSnapshot.mNonActivePlans.size());
            }
            return true;
        }
        if (InEvent == Event::Character('N'))
        {
            ...
            if (!mSnapshot.mNonActivePlans.empty())
            {
                if (mSelectedNonActiveIndex < 0)
                {
                    mSelectedNonActiveIndex = static_cast<int>(mSnapshot.mNonActivePlans.size());
                }
                mSelectedNonActiveIndex = (mSelectedNonActiveIndex - 1 + static_cast<int>(mSnapshot.mNonActivePlans.size())) % static_cast<int>(mSnapshot.mNonActivePlans.size());
            }
            return true;
        }
        ...
        if (mbActiveBlockFocused && mSelectedPlanIndex >= 0 && mSelectedPlanIndex < static_cast<int>(mSnapshot.mActivePlans.size()))
        {
            KeySelectedPlan = mSnapshot.mActivePlans[static_cast<size_t>(mSelectedPlanIndex)];
        }
        else if (!mbActiveBlockFocused && mSelectedNonActiveIndex >= 0 && mSelectedNonActiveIndex < static_cast<int>(mSnapshot.mNonActivePlans.size()))
        {
            KeySelectedPlan = mSnapshot.mNonActivePlans[static_cast<size_t>(mSelectedNonActiveIndex)];
        }
        ...
        if (InEvent == Event::Character('P'))
        {
            ...
            if (PhaseCount > 0)
            {
                ...
                mSelectedPhaseIndex = (mSelectedPhaseIndex - 1 + PhaseCount) % PhaseCount;
                mSelectedLaneIndex = -1;
                mFilePageIndex = 0;
            }
            return true;
        ...
        if (InEvent == Event::Character('w'))
        {
            ...
            if (mSelectedPhaseIndex >= 0 && mSelectedPhaseIndex < static_cast<int>(KeySelectedPlan.mPhases.size()))
            {
                const std::string& PhKey = KeySelectedPlan.mPhases[static_cast<size_t>(mSelectedPhaseIndex)].mPhaseKey;
        ...
        if (InEvent == Event::Character('W'))
        {
            ...
            if (rpTax != nullptr && rpTax->mWaveCount > 0)
            {
                mSelectedWaveIndex = (mSelectedWaveIndex - 1 + rpTax->mWaveCount) % rpTax->mWaveCount;
            }
            return true;
        }
        if (InEvent == Event::Character('l'))
        {
            ...
            {
                mSelectedLaneIndex = (mSelectedLaneIndex + 1) % static_cast<int>(rpTax->mLanes.size());
            }
            return true;
        }
        if (InEvent == Event::Character('L'))
        ...
        if (InEvent == Event::Character('f'))
        {
            ...
            {
                const int PageCount = (static_cast<int>(rpTax->mFileManifest.size()) + 9) / 10;
    ...
    mDataThread = std::thread([this, &screen] {
        while (mRunning.load(std::memory_order_relaxed))
        {
            ...
            if (!mbForceRefresh && mTickCount > 0)
            {
                ...
                TryComputeMarkdownCorpusSignature(RepoRoot, NewSignature, SignatureError);
            ...
            FDocWatchSnapshot Fresh = BuildWatchSnapshot(mRepoRoot, mbUseCache, mConfig.mCacheDir, false);
