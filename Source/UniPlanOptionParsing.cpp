...
ConsumeCommonOptions(const std::vector<std::string> &InTokens,
  ...
  for (size_t Index = 0; Index < InTokens.size(); ++Index) {
    ...
    if (Token == "--text") {
      throw UsageError("`--text` was removed in doc 3.0.0. Use default JSON "
                       "output or `--human`.");
    }
    if (Token == "--human") {
...
ListOptions ParseListOptions(const std::vector<std::string> &InTokens) {
  ...
  for (size_t Index = 0; Index < Remaining.size(); ++Index) {
    ...
    if (Token == "--type") {
      Options.mKind = ToLower(ConsumeValuedOption(Remaining, Index, "--type"));
      continue;
    }
...
int ParsePositiveInteger(const std::string &InValue,
  ...
  try {
    ...
    throw UsageError("Value out of range for " + InOptionName + ": " + InValue);
  }
}
...
EvidenceOptions ParseEvidenceOptions(const std::vector<std::string> &InTokens,
  ...
  for (size_t Index = 0; Index < Remaining.size(); ++Index) {
    ...
    if (Token == "--phase") {
      Options.mPhaseKey = ConsumeValuedOption(Remaining, Index, "--phase");
      continue;
    }
...
SchemeOptions ParseSchemeOptions(const std::vector<std::string> &InTokens) {
  ...
  if (!IsSupportedSchemaType(Options.mType)) {
    throw UsageError(
        "Invalid value for --type: " + Options.mType +
        ". Supported values: doc | plan | playbook | implementation");
  }
  return Options;
}
...
ParseSectionResolveOptions(const std::vector<std::string> &InTokens) {
  ...
  for (size_t Index = 0; Index < Remaining.size(); ++Index) {
    ...
    if (Token == "--section") {
      Options.mSection = ConsumeValuedOption(Remaining, Index, "--section");
      continue;
    }
  ...
  for (size_t Index = 0; Index < Remaining.size(); ++Index) {
    ...
    if (Token == "--line-char-limit") {
      ...
      try {
        ...
        throw UsageError("Invalid value for --line-char-limit: " + Value);
      }
  ...
  for (size_t Index = 0; Index < Remaining.size(); ++Index) {
    ...
    if (Token == "--table-id") {
      Options.mTableId = ParsePositiveInteger(
          ConsumeValuedOption(Remaining, Index, "--table-id"), "--table-id");
      continue;
    }
...
ParseCacheConfigOptions(const std::vector<std::string> &InTokens) {
  ...
  for (size_t Index = 0; Index < Remaining.size(); ++Index) {
    ...
    if (Token == "--enabled") {
      Options.mEnabled = ConsumeValuedOption(Remaining, Index, "--enabled");
      std::string Val = Options.mEnabled;
      std::transform(Val.begin(), Val.end(), Val.begin(), ::tolower);
      if (Val != "true" && Val != "false") {
        throw UsageError("Invalid value for --enabled (expected true|false): " +
                         Options.mEnabled);
      }
    ...
    if (Token == "--verbose") {
      Options.mVerbose = ConsumeValuedOption(Remaining, Index, "--verbose");
      std::string Val = Options.mVerbose;
      std::transform(Val.begin(), Val.end(), Val.begin(), ::tolower);
      if (Val != "true" && Val != "false") {
        throw UsageError("Invalid value for --verbose (expected true|false): " +
                         Options.mVerbose);
      }
  ...
  if (!bHasAnyField) {
    throw UsageError(
        "cache config requires at least one of --dir, --enabled, or --verbose");
  }
