---
name: upl-claude-audit
description: Audit the entire .claude/ system for integrity and consistency. Use this skill to verify skills, hooks, rules, and settings.json are accurate, up-to-date, and consistent with CLAUDE.md, CODING.md, and NAMING.md. Detects stale references, wrong conventions, missing files, frontmatter errors, and duplications.
implicit_invocation: true
---

# UPL Claude Audit

Use this skill to audit the `.claude/` system for integrity, consistency, and accuracy.

## Scope

Audits all `.claude/` artifacts:
- **Skills**: `.claude/skills/*/SKILL.md` — frontmatter, referenced paths, build commands, conventions
- **Hooks**: `.claude/hooks/*.sh` — JSON parsing patterns, exception files, exit codes
- **Rules**: `.claude/rules/*.md` — consistency with CODING.md, NAMING.md, CLAUDE.md
- **Settings**: `.claude/settings.json` — all referenced hooks exist, valid JSON
...
- `CLAUDE.md` — project structure, skill registry, build commands
...
```bash
test -x ".claude/hooks/<hookname>.sh" && echo "OK"
```
...
All scripts under `.claude/` must use the `upl-` or `upl_` prefix.
