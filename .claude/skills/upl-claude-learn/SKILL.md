---
name: upl-claude-learn
description: Learn from the current session and propose updates to the .claude/ system. Use this skill after a productive session to capture new patterns, conventions, anti-patterns discovered, and rules violated. Proposes changes to skills, hooks, rules, and CLAUDE.md for user review.
implicit_invocation: true
---

# UPL Claude Learn

Use this skill after a productive session to analyze what happened and propose improvements to the `.claude/` system.
...
| **New Rule** | New `.claude/rules/` file | User approval |
| **New Hook** | New `.claude/hooks/` script + settings.json entry | User approval |
| **New Skill** | New `.claude/skills/` directory | User approval |
...
- If it's a new hook: update `.claude/settings.json`

### 6. Verify

After applying changes, run `upl-claude-audit` to verify system consistency.
...
- **Verify after**: Always run `upl-claude-audit` after applying changes
