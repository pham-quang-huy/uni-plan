...
| Snippet anti-pattern lint clean | No rejected code shapes in `code_snippets`, `code_entity_contract`, or `best_practices` | `python3 .claude/hooks/plan_snippet_antipattern.py --topic <T> --phase <N> --strict` |
...
- **`--ack-only` on every mutation** emitting `uni-plan-mutation-v1`
  ...
  `uni-plan-sync-execution-v1` schema and silently ignores
  `--ack-only`.
