...
1. **LONG-TERM FIX**: Never use workarounds, quick hacks, or backward-compat shims. Fix the root cause. If a bug is caused by a design flaw, fix the design — do not patch the caller.
   ...
   - **NO THRESHOLD-SLIDING**: When a validator, gate, or threshold you just tightened fires on existing data, the threshold is NOT the responsible layer — **the data is.** Lowering a threshold in response to observed fire volume is threshold-sliding — the exact anti-pattern as changing a failing test to match buggy code. Forbidden tells, any of which make the change a workaround:
     1. The proposed new threshold is numerically close to the observed failing values ("fitting to escape").
     2. The justification for lowering is discovered *after* running the gate ("post-hoc rationalization" — e.g. "V4 is denser than V3").
     3. The author surveys the failing items, concludes "they look okay to me," and weakens the gate to accept them.
   - When a newly-tightened gate fires, the only non-workaround responses are: (a) author real content so items meet the bar, or (b) demote status / change kind so the items no longer claim the contract the gate enforces. Threshold changes must come from *first principles before data is observed*, never as a reaction to fire volume.
   - If the threshold genuinely is wrong, the argument must be derivable without reference to the firing items — e.g. a new measurement framework, a published standard, or a contract change the user explicitly ratifies. Even then, propose the change; do not land it unilaterally as part of fixing the fired items.
