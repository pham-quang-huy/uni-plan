...
1. `--ack-only` on every mutation command. Switches the response
   ...
   programmatic mutation; save the default shape for human-debugging.
   Bundle persistence, auto-changelog, and exit codes are unaffected.
   Example:
...
4. `--all-phases` on `phase readiness` / `phase get` / `phase metric`.
   ...
   creation to sweep readiness across every phase you just authored:
